import { z } from "zod";
import { inputFieldSchema } from "../validatonSchema/schema";

export type CreditData = z.infer<typeof inputFieldSchema>;
...
export function calculateCreditScore(data: CreditData): number {
  ...
  const usageRatio =
    data.creditCardUsage.currentUsage / data.creditCardUsage.totalLimit;
